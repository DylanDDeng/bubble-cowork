...
import { loadMiniMaxTtsConfig, saveMiniMaxTtsConfig } from './libs/minimax-tts-config';
...
async function fetchYouTubeTranscript(url: string): Promise<YouTubeTranscriptResult> {
  const tempDir = await fsPromises.mkdtemp(join(tmpdir(), 'aegis-podcast-'));

  try {
    let title: string | undefined;
    try {
      const titleResult = await execFileAsync('yt-dlp', ['--skip-download', '--print', '%(title)s', url], {
        timeout: 30_000,
        maxBuffer: 1024 * 1024,
      });
      title = titleResult.stdout.trim() || undefined;
    } catch {
      title = undefined;
    }
    ...
    try {
      ...
      // yt-dlp can exit non-zero after partial subtitle downloads; we still inspect tempDir below.
    }

    const files = await fsPromises.readdir(tempDir);
    const subtitleFile =
      files.find((file) => file.endsWith('.en.json3')) ||
      files.find((file) => file.endsWith('.json3')) ||
      files.find((file) => file.endsWith('.en.vtt')) ||
      files.find((file) => file.endsWith('.vtt'));

    if (!subtitleFile) {
      throw new Error('No transcript or subtitles were available for this YouTube video.');
    }

    const rawTranscript = await fsPromises.readFile(join(tempDir, subtitleFile), 'utf8');
    const transcript = subtitleFile.endsWith('.json3')
      ? parseJson3Transcript(rawTranscript)
    ...
    const language = subtitleFile.split('.').slice(-2, -1)[0] || undefined;
...
interface PodcastSpeakerSegment {
...
function logPodcastAudio(message: string, extra?: Record<string, unknown>): void {
  if (extra) {
    console.log(`[Podcast Audio] ${message}`, extra);
    return;
  }

  console.log(`[Podcast Audio] ${message}`);
}
...
function matchPodcastSpeakerLine(
  line: string,
): { speaker: PodcastSpeaker; text: string } | null {
  const normalizedLine = line.trim();
  if (!normalizedLine) {
    return null;
  }

  const speakerPatterns: Array<{ speaker: PodcastSpeaker; pattern: RegExp }> = [
    {
      speaker: 'hostA',
      pattern:
        /^(?:#{1,6}\s*)?(?:>\s*)?(?:[-*]\s*)?(?:\*\*|__)?\s*(?:主持人\s*A|Host\s*A)(?:\s*[（(][^）)]*[）)])?\s*[:：]\s*(?:\*\*|__)?\s*(.*)$/i,
    },
    {
      speaker: 'hostB',
      pattern:
        /^(?:#{1,6}\s*)?(?:>\s*)?(?:[-*]\s*)?(?:\*\*|__)?\s*(?:主持人\s*B|Host\s*B)(?:\s*[（(][^）)]*[）)])?\s*[:：]\s*(?:\*\*|__)?\s*(.*)$/i,
    },
  ];

  for (const { speaker, pattern } of speakerPatterns) {
...
function parsePodcastSpeakerSegments(script: string): PodcastSpeakerSegment[] {
  const segments: PodcastSpeakerSegment[] = [];
  let activeSpeaker: PodcastSpeaker | null = null;
  ...
  const flush = () => {
    ...
    const text = stripMarkdownForTts(activeLines.join(' ').trim());
    if (text) {
      segments.push({ speaker: activeSpeaker, text });
    }
  ...
  for (const rawLine of script.split(/\r?\n/)) {
    ...
    const speakerMatch = matchPodcastSpeakerLine(line);
    if (speakerMatch) {
      flush();
      activeSpeaker = speakerMatch.speaker;
      activeLines = speakerMatch.text ? [speakerMatch.text] : [];
      continue;
    }
...
function normalizeMiniMaxConfig(config?: Partial<MiniMaxTtsConfig> | null): MiniMaxTtsConfig {
  return {
    apiKey: typeof config?.apiKey === 'string' ? config.apiKey.trim() : '',
  };
}

function normalizePodcastAudioInput(input: GeneratePodcastAudioInput): GeneratePodcastAudioInput {
  return {
    draftId: typeof input.draftId === 'string' ? input.draftId.trim() : '',
    title: typeof input.title === 'string' ? input.title.trim() : '',
    script: typeof input.script === 'string' ? input.script : '',
    hostAVoiceId: typeof input.hostAVoiceId === 'string' ? input.hostAVoiceId.trim() : '',
    hostBVoiceId: typeof input.hostBVoiceId === 'string' ? input.hostBVoiceId.trim() : '',
    speed:
    ...
    volume:
      typeof input.volume === 'number' && Number.isFinite(input.volume)
        ? Math.min(10, Math.max(0.1, input.volume))
        : 1,
    pitch:
      typeof input.pitch === 'number' && Number.isFinite(input.pitch)
        ? Math.min(12, Math.max(-12, input.pitch))
        : 0,
    format: 'mp3',
  };
}

async function minimaxJsonRequest<T>(
  path: string,
  apiKey: string,
  init?: RequestInit,
): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorText || `MiniMax request failed with status ${response.status}.`);
  }

  return (await response.json()) as T;
}

async function createMiniMaxAudioTask(
  apiKey: string,
  voiceId: string,
  text: string,
  ...
  if (result.base_resp?.status_code !== 0 || !result.task_id) {
    throw new Error(result.base_resp?.status_msg || 'Failed to create MiniMax audio task.');
  }

  return result.task_id;
}

async function waitForMiniMaxFileId(
  apiKey: string,
  taskId: number,
  ...
  for (let attempt = 0; attempt < MINIMAX_MAX_POLL_ATTEMPTS; attempt += 1) {
    const result = await minimaxJsonRequest<{
      ...
      file_id?: number;
      base_resp?: { status_code?: number; status_msg?: string };
    }>(`/v1/query/t2a_async_query_v2?task_id=${taskId}`, apiKey, { method: 'GET' });

    if (result.base_resp?.status_code !== 0) {
      throw new Error(result.base_resp?.status_msg || 'Failed to query MiniMax task status.');
    }
...
async function downloadMiniMaxAudio(apiKey: string, fileId: number, outputPath: string): Promise<void> {
  const response = await fetch(`${MINIMAX_API_BASE}/v1/files/retrieve_content?file_id=${fileId}`, {
    headers: {
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => '');
    throw new Error(errorText || `Failed to download MiniMax audio file ${fileId}.`);
  }
...
async function concatPodcastAudioSegments(segmentPaths: string[], outputPath: string): Promise<void> {
  const concatListPath = join(tmpdir(), `aegis-podcast-concat-${uuidv4()}.txt`);
  const concatListContent = segmentPaths
    .map((segmentPath) => `file '${segmentPath.replace(/'/g, "'\\''")}'`)
    .join('\n');

  await fsPromises.writeFile(concatListPath, concatListContent, 'utf8');
  ...
  try {
    ...
    await fsPromises.rm(concatListPath, { force: true });
  }
}

async function synthesizePodcastSegment(
  apiKey: string,
  input: GeneratePodcastAudioInput,
  segment: PodcastSpeakerSegment,
  index: number,
  total: number,
  segmentsDir: string,
): Promise<string> {
  const voiceId = segment.speaker === 'hostA' ? input.hostAVoiceId : input.hostBVoiceId;
  const segmentNumber = index + 1;
  logPodcastAudio(`Starting segment ${segmentNumber}/${total}`, {
    speaker: segment.speaker,
    voiceId,
    chars: segment.text.length,
  });

  const taskId = await createMiniMaxAudioTask(apiKey, voiceId, segment.text, input);
  logPodcastAudio(`Created MiniMax task for segment ${segmentNumber}/${total}`, { taskId });

  const fileId = await waitForMiniMaxFileId(apiKey, taskId, {
    index: segmentNumber,
    total,
  });
  const segmentPath = join(segmentsDir, `${String(segmentNumber).padStart(3, '0')}.mp3`);
  await downloadMiniMaxAudio(apiKey, fileId, segmentPath);

  logPodcastAudio(`Finished segment ${segmentNumber}/${total}`, {
    taskId,
    fileId,
    segmentPath,
  });
...
async function generatePodcastAudio(input: GeneratePodcastAudioInput): Promise<GeneratePodcastAudioResult> {
  const normalizedInput = normalizePodcastAudioInput(input);
  const config = normalizeMiniMaxConfig(loadMiniMaxTtsConfig());
  if (!config.apiKey) {
    throw new Error('Please save your MiniMax API key before generating audio.');
  }

  if (!normalizedInput.draftId) {
    throw new Error('Missing podcast draft id.');
  }

  if (!normalizedInput.script.trim()) {
    throw new Error('Please generate or paste a podcast script first.');
  }

  if (!normalizedInput.hostAVoiceId || !normalizedInput.hostBVoiceId) {
    throw new Error('Please choose voices for both hosts.');
  }

  const segments = parsePodcastSpeakerSegments(normalizedInput.script);
  if (segments.length === 0) {
    throw new Error('Script must use “主持人A：” and “主持人B：” speaker labels before generating audio.');
  }

  const baseName = sanitizeFileName(normalizedInput.title || 'podcast-episode') || 'podcast-episode';
  const outputDir = join(app.getPath('userData'), 'podcast-audio', normalizedInput.draftId, `${Date.now()}`);
  const segmentsDir = join(outputDir, 'segments');
  await fsPromises.mkdir(segmentsDir, { recursive: true });

  logPodcastAudio('Audio generation started', {
    draftId: normalizedInput.draftId,
    title: normalizedInput.title,
    segmentCount: segments.length,
    concurrency: MINIMAX_SEGMENT_CONCURRENCY,
  });

  const segmentPaths = new Array<string>(segments.length);

  try {
    let nextIndex = 0;
    const workerCount = Math.min(MINIMAX_SEGMENT_CONCURRENCY, segments.length);
    ...
    await Promise.all(
      Array.from({ length: workerCount }, async () => {
        while (nextIndex < segments.length) {
          ...
          nextIndex += 1;
          segmentPaths[currentIndex] = await synthesizePodcastSegment(
            config.apiKey,
            normalizedInput,
    ...
    logPodcastAudio('All segments downloaded, concatenating final MP3', { outputPath });
    await concatPodcastAudioSegments(segmentPaths, outputPath);
    logPodcastAudio('Audio generation completed', { outputPath, segmentCount: segmentPaths.length });
    ...
    logPodcastAudio('Audio generation failed', {
...
export function setupIPCHandlers(mainWindow: BrowserWindow): void {
  ...
  ipcMainHandle('generate-podcast-script', async (_, payload: SessionStartPayload) => {
    return handleSessionStart(mainWindow, payload);
  });

  ipcMainHandle('fetch-youtube-transcript', async (_event, url: string) => {
    return fetchYouTubeTranscript(url);
  });

  ipcMainHandle('get-minimax-tts-config', async () => {
    return loadMiniMaxTtsConfig();
  });

  ipcMainHandle('save-minimax-tts-config', async (_event, config: MiniMaxTtsConfig) => {
    return saveMiniMaxTtsConfig(normalizeMiniMaxConfig(config));
  });

  ipcMainHandle('generate-podcast-audio', async (_event, input: GeneratePodcastAudioInput) => {
    return generatePodcastAudio(input);
  });
