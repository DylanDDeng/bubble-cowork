...
> [!NOTE]
> 归档于 2026-07-24。本文记录的是早期产品路线与设计探索，已不再作为当前实现或实施计划的依据。
> 当前架构与实现状态请以 `docs/` 中的现行文档及代码为准。
...
可以提供 starter profiles，但它们不是不可变的内置成员。用户必须能新增、编辑、禁用、删除 agent profile。

starter profiles 可以包括：
...
`Settings -> Agents` 是全局 agent profile 的配置入口。DMs tab 不承载 profile editor，只展示已启用的 global agents，保持“进入私聊”的单一职责。
...
type AgentAvatar = {
  type: 'asset';
  key: 'notion-avatar-01' | 'notion-avatar-02' | 'notion-avatar-03' | 'notion-avatar-04' | 'notion-avatar-05';
};
...
### Phase 2：Configurable Agent Profiles + DM Shell

目标：让 agent 成为用户可配置的 workspace member，而不是写死在 UI 里的默认成员。

范围：

- 新增可持久化 `AgentProfile` 数据模型。
- 提供 starter profiles，但用户可以新增、编辑、禁用、删除。
- profile 包含 name、role、description、instructions、avatar asset、provider、model、permission policy、color、enabled。
- Chat sidebar 增加 `Projects / DMs` tab。
- `Settings -> Agents` 承载 global profile 的新增、编辑、禁用、删除和 runtime 配置。
- `DMs` tab 从用户配置读取 global agent profiles。
- `DMs` tab 只作为私聊入口，不直接放 profile editor。
- Project server 内根据启用的 profiles 显示 agent roster summary，例如 `3 agents active`。
- 右侧 inspector 能显示 agent profile。
- 加入 DM shell，但第一版不把 DM 接到 project cwd runner。
