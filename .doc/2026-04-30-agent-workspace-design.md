...
目标：先把现有 chat sessions 按 project channel 组织起来，不改变执行语义。
...
- 新增 `activeChannelId`。
- Sidebar 显示 channels。
- 现有和新建 sessions 默认归入 `#all`。
- 保持当前 new session 流程可用。

验收：

- 打开项目后能看到 `#all`。
- 老 sessions 不丢失。
- 在某个 channel 新建聊天，会绑定到该 channel。
- 不引入 task/run UX。
...
- 后续允许移动 session/run 到其他 channel。

### 风险：状态迁移复杂

应对：

- 给 session 加 metadata，不替换 session。
- 老 session 默认进入 `#all`。
