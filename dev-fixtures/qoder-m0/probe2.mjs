/**
 * M0 probe round 2 for @qoder-ai/qoder-agent-sdk (v1.0.15)
 * Auth: qodercliAuth(). Results -> probe-report-2.json (does NOT touch probe-report.json).
 *
 * Fixes methodology from round 1 and adds load-bearing adapter probes:
 *  P5-fix  interrupt only after first stream_event; precise timing + full result dump
 *  P6c     canUseTool with a classifier-blocked command (rm -rf nonexistent /tmp dir)
 *          allow / deny / permissionMode matrix (auto, dontAsk)
 *  P7r     getAvailableModels timing matrix: after initializationResult(), after first result
 *  P9      same-Query multi-turn via push AsyncIterable (never returns)
 *  P9b     same-Query interrupt then continue on the SAME Query
 *  P9c     mid-turn priority:'now' message injection (observational)
 *  P10     setModel mid-session
 *  P11     setPermissionMode mid-session (default -> plan -> default)
 *  P12     full system.init dump + full modelUsage dump + token-zero investigation (auto vs fixed model)
 *  P8b     image with isVl=false model vs isVl=true model
 */
