...
* 在同一个界面里切换 Claude Code、Codex、OpenCode、Kimi、Grok、Pi 与 Qoder
...
从 [GitHub Releases](https://github.com/DylanDDeng/bubble-cowork/releases) 下载对应平台的最新构建。

### macOS 提示“应用已损坏”

发布包未做代码签名，安装后 macOS Gatekeeper 可能提示应用已损坏、无法打开。执行一次下面的命令移除隔离属性即可正常打开：

```bash
xattr -cr /Applications/Aegis.app
```

每次更新后重新下载的包会再次被隔离，需要重新执行一次。
